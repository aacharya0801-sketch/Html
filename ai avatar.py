...
fromhuggingface hub import InferenceClient
import config
st.set_page_config(page_title="AI Avatar Creator",page_icons="🎨", layout="centered")
OPTIONS = {
...
st.write("Create your own avatat with AI!")
...
if mode == "Use Avatar Builder":
    values = {k: st.selecbox(f"Choose {k}", v) for k, v in OPTIONS.items()}
    ...
    f"{values['art style']}, colorful, highly detailed, digital art"
)
    final_prompt= f"{prompt}, {extra}" if extra else prompt
else:
    final_prompt = st.text_area(
    "Write your own prompt",
    placeholder="Example: A brave young wizard with silver hair, glowing green eyes, a floating spell book, standing in a magical forest, fantasy illustration",
    height=150,
    ).strip()
...
if st.button("✨ Generate Avatar"):
    if not config.HF_API_KEY:
        ...
        with st.spinner("Creating your avatar..."):
            try:
                st.session_state.generated_image = client.text_to_image(prompt=final_prompt,model=config.HF_IMAGE_MODEL,)
